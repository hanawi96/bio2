...
-- Get IDs for foreign keys and create test data
DO $$
DECLARE
  v_user_id BIGINT;
  v_theme_id BIGINT;
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM bio_pages WHERE user_id = v_user_id AND title = 'My Bio') THEN
    -- Insert test page
    ...
    -- Generate stable ETag from compiled_json (SHA256, first 32 hex chars)
    v_etag := encode(sha256(v_compiled_json::bytea), 'hex');
    v_etag := substring(v_etag from 1 for 32);

    -- Insert publish cache with stable etag
    INSERT INTO page_publish_cache (page_id, compiled_json, etag)
    VALUES (v_page_id, v_compiled_json::jsonb, v_etag);
      
    RAISE NOTICE 'Seed data created successfully! ETag: %', v_etag;
  ELSE
